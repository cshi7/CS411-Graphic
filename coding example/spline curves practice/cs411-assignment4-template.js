...
function tensionUp() {
  tension += 0.1;
  //if we have 4 points or more
  if((ctrlPts.length/2)>3){
	// we go from nCtrlPts = 4, which is the beginning, to re-calculate all the interpolate point
	intrPts=[];
	// -> var i = 0, not int i = 0
	// -> intrPts.length, not length(interpolate)
	for(var nCtrlPts = 4;nCtrlPts<=ctrlPts.length/2;nCtrlPts++){
...
function tensionDown() {
  tension -= 0.1;
  if((ctrlPts.length/2)>3){
	// we go from nCtrlPts = 4, which is the beginning
	intrPts=[];
	for(var nCtrlPts = 4;nCtrlPts<=ctrlPts.length/2;nCtrlPts++){
		var pk_n1 = {x: ctrlPts[nCtrlPts*2-8], y: ctrlPts[nCtrlPts*2-7]}; // p_{k-1}
		var pk    = {x: ctrlPts[nCtrlPts*2-6], y: ctrlPts[nCtrlPts*2-5]}; // p_{k}
...
///////////////////////////////////////////////////////////////////////////////////////
  ...
  for(var u=0; u<=1; u+=uStep){
    ...
    var x = ((0-tension)*u*u*u+2*tension*u*u-tension*u)*(pk_n1.x)+((2-tension)*u*u*u+(tension-3)*u*u+1)*(pk.x)+
((tension-2)*u*u*u+(3-2*tension)*u*u+tension*u)*(pk_p1.x)+(tension*u*u*u-tension*u*u)*(pk_p2.x);
	
	var y = ((0-tension)*u*u*u+2*tension*u*u-tension*u)*(pk_n1.y)+((2-tension)*u*u*u+(tension-3)*u*u+1)*(pk.y)+
((tension-2)*u*u*u+(3-2*tension)*u*u+tension*u)*(pk_p1.y)+(tension*u*u*u-tension*u*u)*(pk_p2.y);
